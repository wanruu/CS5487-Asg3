...
def rgb2ycbcr(img):
    ...
    """
    tmp_list = [65.481, 128.553, 24.966, -37.797, -74.203, 112, 112, -93.786, -18.214]
    oriT = np.asarray(tmp_list).reshape((3, 3), order="C") / 255
    offset = [16, 128, 128]
...
def getfeatures(img, stepsize, follow_matlab = True):
    ...
    # Info of img #
    # ----------- #
    sy, sx, sc = img.shape  # height, width, color
    ...
    py2mat_constant = 1 if follow_matlab else 0  # to compensate the fact that python is 0-index
    i = 0
    for x in rangex:
        for y in rangey:
            myIu = yimg[y:y+winsize, x:x+winsize, 1]
            myIv = yimg[y:y+winsize, x:x+winsize, 2]
            X[:, i] = [
                np.mean(myIu.flatten()), 
